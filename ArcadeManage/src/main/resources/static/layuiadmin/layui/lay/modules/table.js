;layui.define(["laytpl", "laypage", "layer", "form", "util"], function (e) {
    ...
    var t = layui.$, i = layui.laytpl, a = layui.laypage, l = layui.layer, n = layui.form,
        ...
        W = "layui-table-edit", _ = "layui-table-hover", E = function (e) {
            ...
            (), "{{# var isSort = !(item2.colGroup) && item2.sort; }}", '<th data-field="{{ item2.field||i2 }}" data-key="{{d.index}}-{{i1}}-{{i2}}" {{# if( item2.parentKey){ }}data-parentkey="{{ item2.parentKey }}"{{# } }} {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} ' +
            t + ' {{# if(item2.unresize || item2.colGroup){ }}data-unresize="true"{{# } }} class="{{# if(item2.hide){ }}layui-hide{{# } }}{{# if(isSort){ }} layui-unselect{{# } }}{{# if(!item2.field){ }} layui-table-col-special{{# } }}">', '<div class="layui-table-cell laytable-cell-', "{{# if(item2.colGroup){ }}", "group", "{{# } else { }}", "{{d.index}}-{{i1}}-{{i2}}", '{{# if(item2.type !== "normal"){ }}', " laytable-cell-{{ item2.type }}", "{{# } }}", "{{# } }}", '" {{#if(item2.align){}}align="{{item2.align}}"{{#}}}>', '{{# if(item2.type === "checkbox"){ }}', '<input type="checkbox" name="layTableCheckbox" lay-skin="primary" lay-filter="layTableAllChoose" {{# if(item2[d.data.checkName]){ }}checked{{# }; }}>', "{{# } else { }}", '<span>{{item2.title||""}}</span>', "{{# if(isSort){ }}", '<span class="layui-table-sort layui-inline"><i class="layui-edge layui-table-sort-asc" title="升序"></i><i class="layui-edge layui-table-sort-desc" title="降序"></i></span>', "{{# } }}", "{{# } }}", "</div>", "</th>", e.fixed ? "{{# }; }}" : "", "{{# }); }}", "</tr>", "{{# }); }}", "</thead>", "</table>"].join("")
        },
    ...
    I.prototype.config = {
        ...
        if ("default" === a.toolbar)
            n.html(l); else if ("string" == typeof a.toolbar) {
        ...
        if (i.startTime = (new Date).getTime(), a.url) {
            ...
            a.contentType && 0 == a.contentType.indexOf("application/json") && (d = JSON.stringify(d)), i.loading(), t.ajax({
                ...
                headers: a.headers || {},
                success: function (t) {
                    "function" == typeof a.parseData && (t = a.parseData(t) || t), t[n.statusName] != n.statusCode ? (i.renderForm(), i.errorView(t[n.msgName] || '返回的数据不符合规范，正确的成功状态码应为："' +
        ...
        if (i.totalRow) {
            ...
            t.eachCols(function (e, t) {
                var n = t.field || e, o = ['<td data-field="' + n + '" data-key="' + i.index + "-" + t.key + '" ' +
                function () {
                    var e = [];
            ...
            switch (layui.stope(e), F.trigger("table.tool.panel.remove"), l.close(a.tipsIndex), c) {
                ...
                case"LAYTABLE_EXPORT":
                    r.ie ? l.tips("导出功能不支持 IE，请用 Chrome 等高级浏览器导出", this, {tips: 3}) : s({
                        list: function () {
                            return ['<li data-type="xls">导出单页</li>', '<li data-type="all">导出全部</li>'].join("")
                        }
                        (), done: function (e, i) {
                            i.on("click", function () {
                                ...
                                if (e == 'all') {
                                    layui.$.ajax(o.url, {
                                        data: {excel: true, searchData: o.where['searchData']},
                                        dataType: 'json',
                                        type: 'get',
                                        timeout: 10000,
                                        success: (data) => {
                                            ...
                                            layer.msg("请求成功");
                                            d.export(o.id, data.data);
                                        },
                                        error: function (xhr, type, errorThrown) {
                                            layer.msg('导出excel发生异常~');
                                        }
                                    });
                                } else
        ...
        var g = "layui-table-grid-down", b = function (e) {
            ...
            if (!i.data("off"))
                if (e)
                    i.find(".layui-table-grid-down").remove(); else if (a.prop("scrollWidth") > a.outerWidth()) {
                    if (a.find("." + g)[0])
                        return;
    ...
    }, d.export = function (e, t) {
        let i = "csv";
        let l = {csv: "text/csv", xls: "application/vnd.ms-excel"};
        let a = c.config[e] || {};
        if (r.ie) {
        ...
        n.download = (document.title || "table_" + (a.index || "")) + ".xls";
