...
public class IPSeeker
{
    ...
    private static final String REDIS_KEY = "QQWry";

    /**
     * 进行引入redis
     */
    public static void setIpCache(String ip, IPLocation location)
    {
        RedisUtils.hset(REDIS_KEY, ip, JSONObject.toJSONString(location));
    }
     ...
     * @return 存储地址
     */
    public static IPLocation getIpCache(String ip)
    {
        String value = RedisUtils.hget(REDIS_KEY, ip);
        if (value != null)
            return CmTool.parseJSONByFastJSON(value, IPLocation.class);
        return null;
    }
    ...
    private IPSeeker()
    {
        String field = "wry-version";
        String wry_version = RedisUtils.hget(REDIS_KEY, field);
        String version = ReadConfig.get(field);
        if (!version.equals(wry_version))
        {
            RedisUtils.del(REDIS_KEY);
            RedisUtils.hset(REDIS_KEY, field, version);
        }
    ...
    public List<IPEntry> getIPEntriesDebug(String s)
    {
        ...
        for (long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH)
        {
            ...
            if (temp != -1)
            {
                ...
                readEntry(s, ret, offset, temp, loc);
            }
        }
        return ret;
    }
    ...
    private void readEntry(String s, List<IPEntry> ret, long offset, long temp, IPLocation loc)
    {
        if (loc == null)
            return;
    ...
    private IPLocation getIP(byte[] ip)
    {
        ...
        IPLocation location = getIpCache(ipStr);
        if (location != null)
        {
            return location;
        } else
    ...
    {
        return getIP(Utils.getIpByteArrayFromString(ip));
    }
