...
public class RedisUtils
{
    ...
    static
    {
        try
        {
            ...
            config.setMaxTotal(Integer.valueOf(bundle.getProperty("redis.pool.maxTotal")));
            config.setMaxIdle(Integer.valueOf(bundle.getProperty("redis.pool.maxIdle")));
            config.setMaxWaitMillis(Long.valueOf(bundle.getProperty("redis.pool.maxWait")));
     ...
     * @return
     */
    public static JSONObject zSetTuple(String key, String member)
    {
        ...
        try
        {
            if (jds != null)
            {
                ...
                if (rank != null)
                {
                    tuple.put("index", rank + 1);
                    Double score = jds.zscore(key, member);
                    tuple.put("mark", score);
                }
            }
            if (!tuple.containsKey("index"))
            {
                tuple.put("index", -1);
                tuple.put("mark", 0);
            }
